return {
	{
		...
		config = function()
			...
			dapui.setup({
				...
				mappings = { -- Use a table to apply multiple mappings expand = { "<CR>", "<2-LeftMouse>" }, open = "o", remove = "d", edit = "e",
					repl = "r",
					toggle = "t",
				},
	...
	},
	{
		...
		config = function()
			...
			vim.keymap.set("n", "<Leader><F5>", "<CMD>lua require('dap').continue()<CR>")
			...
			if not present_dap or not present_virtual_text or not present_dap_utils then
				local missing_dependencies = {}
				if not present_dap then
					table.insert(missing_dependencies, "dap")
				end
				if not present_virtual_text then
					table.insert(missing_dependencies, "virtual_text")
				end
				if not present_dap_utils then
					table.insert(missing_dependencies, "dap_utils")
				end

				if #missing_dependencies > 0 then
					error("Missing dap dependencies: " .. table.concat(missing_dependencies, ", "))
				end
			...
			--			keymap("n", "<Leader>dw",
			--				"<CMD>lua require('dapui').float_element('watches', { enter = true })<CR>", opts)
			--			keymap("n", "<Leader>ds",
			--				"<CMD>lua require('dapui').float_element('scopes', { enter = true })<CR>", opts)
			...
			require("dap").defaults.fallback.external_terminal = {
				command = "wt.exe",
				args = { "--profile", "YourProfileName", "--startingDirectory", "." },
			}
			...
			for i, ext in ipairs(exts) do
				...
				dap.configurations[ext] = {
					...
					},
					{
						type = "pwa-node",
						request = "launch",
						name = "no idea waht im doing",
						cwd = vim.fn.getcwd(),
						runtimeArgs = { "run", "startWIN32" },
						runtimeExecutable = "npm",
						sourceMaps = true,
						protocol = "debugger",
						skipFiles = { "<node_internals>/**", "node_modules/**" },
